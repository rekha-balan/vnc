...
namespace CodeGenerationWithRoslyn
{
    public static class Symbols
    {
        ...
        {
            foreach (var member in compilation.Assembly.GlobalNamespace.GetMembers().Where(member => member.CanBeReferencedByName))
            {
                Console.WriteLine(member.Name);

                foreach (var item in member.GetTypeMembers().Where(item => item.CanBeReferencedByName))
                {
                    Console.WriteLine("\t{0}:{1}", item.TypeKind, item.Name);

                    foreach (var innerItem in item.GetMembers().Where(innerItem => innerItem.CanBeReferencedByName))
                    {
